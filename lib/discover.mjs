...
function parseReadme(readmePath, dirName) {
  ...
  const id = titleMatch?.[1] ?? dirName.split('-').slice(0, 2).join('-');
  ...
  const opsLine = lines.find(l => /^\d+\s+ops?\s+in/i.test(l));
...
/**
 * Parse @sentinel and @package annotations from a fix.py/fix.sh header.
 * Lines must be at the top of the file (before any non-comment code).
 */
function parseSentinels(fixPath) {
  const text = readFileSync(fixPath, 'utf-8');
  const lines = text.split('\n');
  ...
  for (const line of lines) {
    if (line.startsWith('# @package:')) {
      ...
      const raw = line.replace('# @sentinel:', '').trim();
      if (raw === 'none') {
        sentinels.push({ type: 'none' });
      } else {
        const absentMatch = raw.match(/^absent\s+"(.+)"\s+(.+)$/);
        if (absentMatch) {
          sentinels.push({ type: 'absent', pattern: absentMatch[1], file: absentMatch[2] });
          continue;
        }
        const grepMatch = raw.match(/^grep\s+"(.+)"\s+(.+)$/);
        if (grepMatch) {
...
export function discover() {
  ...
  for (const dirName of dirs) {
    ...
    const slug = dirName.replace(/^[A-Z]+-\d+-/, '');
    ...
    // Parse sentinel metadata from the fix file
    const fixPath = resolve(PATCH_DIR, dirName, hasPy ? 'fix.py' : 'fix.sh');
    const sentinel = existsSync(fixPath) ? parseSentinels(fixPath) : { package: null, sentinels: [] };
